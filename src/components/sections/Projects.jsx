    ...
    <section id="projects" className="section-pad dot-grid" ref={ref}>
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : {}}
          ...
          <span className="text-xs font-mono tracking-widest text-[#F9731688] uppercase">Projects</span>
        </motion.div>
          ...
          {projectCategories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              className={`px-4 py-1.5 rounded-full text-xs font-mono transition-all duration-200 ${
                activeCategory === cat ? 'bg-[#F97316] text-[#050A0E] font-bold' : 'glass text-white/50 hover:text-white/80'
              }`} data-hover>
              {cat}
            </button>
          ))}
